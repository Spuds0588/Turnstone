...
| `test/bookmarklet.html` | a browser + the repo served | all three payload variants on a page they do not own: which formats each reads, that a workbook in the `csv` variant is refused *by name*, that the host page's own `XLSX`/`Papa` survive, that closing restores the page byte-for-byte, and that a second run refuses. Plus the two things that live inside the shadow root and could plausibly behave differently there — the **☰ menu measured as a full-viewport panel** with its three switches, and the **column panel with an edited cell**: the edit is proved by switching the column back to *Read only*, because the payload is an IIFE whose `buildExportMatrix()` is not reachable from the harness and read-only markup is built straight out of the row. That column block runs in its **own freshly-mounted overlay with the vendor sheet as the first drop** — the canonical queue's four columns are all card features, and a second drop in this harness was measured to be ignored outright by the zero-library build |
...
11. Open **⚙ Columns** on `sample-vendor-onboarding.csv`: the three role columns should read *card feature*
    with their editor dropdowns off, and `Owner`/`Contact` should be draggable and numbered `#1`/`#2`.
    Give `Owner` a **Dropdown**, watch the choices fill from the column's own values, pick one, then set the
    column back to *Read only* and confirm the card shows the value you picked. Rename that column and
    confirm the card label *and* the sort list follow it, then export and confirm the header cell changed in
    the file. Finally press **Reset columns** and confirm the order, the hidden set and the editors are gone.
12. In fallback mode (drop a file rather than picking one), change something, wait a second, and
