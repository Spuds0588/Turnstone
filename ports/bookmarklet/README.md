...
| --- | --- | --- | --- |
| `csv` | ~252 KB | app + a 2 KB RFC-4180 reader | CSV, TSV, JSON, HTML, XML |
| `core` | ~261 KB | app + a read-only XLSX reader | the above **+ XLSX / XLS / ODS reading** |
| `full` | ~1.17 MB | app + vendored PapaParse + SheetJS | every format, plus **XLSX export and write-back** |

The old "< ~8 KB" target is not reachable while keeping the real app: the app shell alone is
~247 KB, and the promise was always "the whole Turnstone app, not a cut-down clone". What is
