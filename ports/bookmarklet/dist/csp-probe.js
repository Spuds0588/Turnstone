(function(){
/* Turnstone bookmarklet — "core" variant, generated 2026-09-25T20:06:51.936Z
   from app.html @ bf426cd + uncommitted changes. Regenerate with: node ports/bookmarklet/build.js --variant core
...
const modalOpen = (sel) => [...ROOT.querySelectorAll(sel)].some((el) => {
...
const currentTheme = () => (THEME_ROOT.dataset.theme === 'light' ? 'light' : 'dark');
...
/** Is this pasted text a **workspace link** — a list already inside a URL — or a
    plain URL for a file to fetch? The answer decides whether the app reads its own
    fragment or makes a network request, so it follows a rule rather than a guess:

      1. A `zdata` payload **anywhere** is unambiguous. Nothing else in the world
         spells a URL parameter that way; it is this app's own name for it.
      2. A payload in the **fragment** is also unambiguous, and for a reason worth
         writing down: a fragment is never sent to a server, so it cannot be a file
         URL's arguments. `x.csv#data=…` can only be about a list someone handed over.
      3. A `data` payload in the **query string** is genuinely ambiguous — plenty of
         file endpoints take `?data=…` — so it counts only when the URL is one of our
         own pages: same origin, path ending in `.html` (or the directory root). That
         is the tooling case, where something can only append a query string.

    Pure string work, so the Node suite tests it without a browser — and every
    refusal carries a reason, because "why didn't my link open?" deserves an answer
    rather than a silent fetch of something that was never a file. */
function workspaceLinkFrom(text) {
  const raw = String(text || '').trim();
  if (!raw) return { ok: false, why: 'there was nothing to open' };

  /* A bare `#zdata=…` or `zdata=…` — copied on its own, which is a normal way to
     pass one on — is read as the fragment it is. */
  const bare = /^(zdata|data|sum|format|name|link|open)=/i.test(raw.replace(/^[#?]+/, ''));
  let url, frag = '', query = '';
  try {
    url = bare ? new URL(location.href) : new URL(raw, location.href);
  } catch (e) {
    return { ok: false, why: 'that is not a link' };
  }
  if (bare) frag = raw.replace(/^[#?]+/, '');
  else { frag = String(url.hash || '').replace(/^#/, ''); query = String(url.search || '').replace(/^\?/, ''); }

  const fromFrag = inlineParamsFrom('', frag);
  const fromQuery = inlineParamsFrom(query, '');
  const params = inlineParamsFrom(query, frag);          // fragment wins, as everywhere else
  const payload = (fromFrag.zdata || fromQuery.zdata || '').trim();
  const plain = (fromFrag.data || fromQuery.data || '').trim();

  if (payload) return { ok: true, params, where: fromFrag.zdata ? 'the fragment' : 'the query string' };
  if (plain && fromFrag.data) return { ok: true, params, where: 'the fragment' };

  /* The tooling case: `?data=` on one of our own pages. A `.csv` or `.json` endpoint
     with a `data` parameter is a file, and stays one. */
  const ours = !bare && url.origin === location.origin
    && (/\/$/.test(url.pathname) || /\.html?$/i.test(url.pathname));
  if (plain && fromQuery.data && ours) return { ok: true, params, where: 'the query string of one of our own pages' };

  /* Nothing to open — but *which* nothing matters to the caller: a file URL should
     still go down the fetch path, while a stray paste should say what was missing. */
  const looksLikeUrl = /^[a-z][a-z0-9+.-]*:\/\//i.test(raw) || /^\//.test(raw) || /^[\w.-]+\.[a-z]{2,}/i.test(raw);
  return looksLikeUrl
    ? { ok: false, why: 'that is a link to a file, not a link that carries the list itself' }
    : { ok: false, why: 'no list is carried in that text — a workspace link has a #zdata=… (or #data=…) part' };
}
...
function checkInlineDigest(raw, text, rawBytes) {
  const claimed = String(raw.sum || '').trim();
  if (!claimed) return { verified: false };
  /* The documented contract (llms.txt, workspace-link.html) hashes the same bytes
     the link deflates. Those are the *raw* bytes here — but `TextDecoder` strips a
     leading UTF-8 BOM, so a generator that built its list from a BOM'd file (Excel
     writes one) hashed three bytes the decoder no longer sees, and a correct link
     was refused as tampered. Verify against the raw bytes first — the contract —
     and fall back to the decoded text, which is what every app-built link has
     carried and what a generator that digests the BOM-less text produces. */
  if (rawBytes && claimed === inlineDigest(rawBytes)) return { verified: true };
...
async function decodeInlineList(raw) {
  ...
  if (z) {
    ...
    /* `TextDecoder` strips a leading UTF-8 BOM by default — the right parse outcome
       (Excel writes BOMs, the sniffer handles the rest) — so the raw inflated bytes
       ride along for the digest check above, which verifies the contract (the bytes
       the link carries) before falling back to the decoded text. */
    const text = new TextDecoder('utf-8').decode(out);
    const { verified } = checkInlineDigest(raw, text, out);
...
async function buildInlineLink(text, opts = {}) {
  ...
  const base = opts.base || `'https://spuds0588.github.io/Turnstone/app.html'`;
...
/* -------------------------- Open a workspace link ------------------------ */
/* The receiving end of the share-link path, and the one door a **side panel** has
   for a list it did not get from a file: a Chrome side panel has no address bar, so
   a `#zdata=` link posted into it has nowhere to arrive. The welcome panel's field
   does this too, but it is hidden the moment a list is loaded — and "Sam sent me
   another queue" is exactly the moment one is already open.

   Nothing here reads the clipboard by itself: a read needs a permission prompt, and
   a prompt that appears because you opened a menu is a prompt people learn to
   dismiss. Pasting into the field is the whole interaction. */
function showOpenLink() {
  ...
  overlay.innerHTML = `
    <div class="modal-box">
      ...
      <input type="text" id="ol-input" class="portal-input" spellcheck="false" autocomplete="off"
             placeholder="https://spuds0588.github.io/Turnstone/app.html#zdata=…">
      <p class="portal-preview" id="ol-hint">A link that carries a list has <code>#zdata=</code> (compressed) or
        <code>#data=</code> (plain text) in it.</p>
      <div class="srow">
        <button class="btn primary" id="ol-open">Open it</button>
        <button class="modal-cancel" id="ol-cancel">Cancel</button>
      </div>
    </div>`;
  mountPoint().appendChild(overlay);
  const input = overlay.querySelector('#ol-input');
  const hint = overlay.querySelector('#ol-hint');
  const close = () => overlay.remove();
  const attempt = async () => {
    const raw = input.value.trim();
    if (!raw) { hint.textContent = 'Paste a link first.'; input.focus(); return; }
    const look = workspaceLinkFrom(raw);
    if (!look.ok) { hint.textContent = `Not a workspace link: ${look.why}.`; return; }
    hint.textContent = 'Opening…';
    try {
      await loadInlineWorkspace(look.params);
      close();
    } catch (e) {
      /* Left open on purpose: the message usually tells you the link was cut short,
         and the obvious next move is to paste the corrected one right here. */
      err('Workspace link could not be opened from the dialog', e);
      hint.textContent = e.message;
    }
  };
  overlay.querySelector('#ol-open').onclick = attempt;
...
function showBuildLink() {
  ...
  overlay.querySelector('#bl-open').addEventListener('click', () => {
    ...
    overlay.remove();
    window.open(built.url, '_blank', 'noopener');   // hashchange loads it
  });
...
/** One entry point for "somebody handed me a string": open the list inside a
    workspace link, or fetch the file the URL points at. The routing rule is
    `workspaceLinkFrom` (pure, tested in Node); this is only the dispatch, so the
    welcome panel's field, the ☰ menu item and the deep-link paths all behave the
    same way. Throws whatever the loader throws — callers own the message. */
async function openPastedSource(raw) {
  const look = workspaceLinkFrom(raw);
  if (look.ok) {
    log(`Pasted a workspace link (the list is in ${look.where}) → opening it rather than fetching anything`);
    await loadInlineWorkspace(look.params);
    return 'link';
  }
  log(`Not a workspace link (${look.why}) → treating it as a file URL`);
...
function closeTabForCompletedRow(i) {
  if (!state.autoCloseOnComplete) return null;
  if (getOpenMode() !== 'tabs') return null;
  if (autoCloseBlockedReason()) { log(`Auto-close skipped for row ${i}: ${autoCloseBlockedReason()}`); return null; }
  const t = tabs.find(x => x.rowId === i);
  if (!t) return null;
  log(`Auto-close: closing tab ${t.id} (${t.title}) with completed row ${i}`);
  closeTab(t.id);
  return t;
}
...
function autoCloseBlockedReason() {
  if (state.linkTemplate) return 'Not for a portal list: every row works in the one shared portal, so closing its tab would pull you out of it.';
  return null;
}
...
els.miOpenLink.addEventListener('click', () => { closeMenu(); showOpenLink(); });
els.miBuild.addEventListener('click', () => { closeMenu(); showBuildLink(); });
els.miLink.addEventListener('click', async () => {
  closeMenu();
  if (state.sourceUrl) {
    const link = `'https://spuds0588.github.io/Turnstone/app.html'?file=${encodeURIComponent(state.sourceUrl)}`;
...
/* The welcome panel's field, which had **no handler at all** — the button and the Enter
   key were both dead, and nothing tested either (the sweeps drive files and pastes, not
   this row). It takes a file URL *or* a workspace link now, and the routing rule decides
   which: a link that carries the list is opened, anything else is fetched. */
const submitWelcomeField = async () => {
  const raw = els.urlInput.value.trim();
  if (!raw) { toast('Paste a file URL or a shared link first', 'error'); els.urlInput.focus(); return; }
  try {
    await openPastedSource(raw);
  } catch (e) {
    err('The welcome field could not open what was pasted', e);
    toast(`That could not be opened: ${e.message}`, 'error');
  }
};
els.btnLoadUrl.addEventListener('click', submitWelcomeField);
els.urlInput.addEventListener('keydown', (e) => { if (e.key === 'Enter') submitWelcomeField(); });
...
(async function init() {
  ...
  /* Point at the other editions — but only where there is a site to point at. The
     standalone build has no versions.html beside it, and the bookmarklet is running
     on somebody else's origin, so both leave the hint hidden. This is the one place
     a user who landed straight on the app can discover the other three. */
  const hasSite = !STANDALONE_BUILD && !SESSION_ONLY;
  els.editionsHint.hidden = !hasSite;
  log(hasSite ? 'Edition pointer shown (other builds available on this site)' : 'Edition pointer hidden (no site beside this build)');
  ...
  const params = new Map();   // bookmarklet: the host page's URL is not ours to read
  ...
  window.addEventListener('hashchange', async () => {
    const p = {};   // bookmarklet: not the host page's fragment either
