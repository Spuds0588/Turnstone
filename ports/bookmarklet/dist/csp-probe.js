...
(function buildManifest() {
  try {
    ...
    const manifest = {
      name: 'Turnstone — URL List Processor',
      short_name: 'Turnstone',
      description: 'Zero-server workspace for link lists (CSV, TSV, XLSX, JSON, HTML, XML): tabbed browsing, status tracking, and write-back — all client-side.',
      start_url: location.pathname,   /* app.html — deep links keep working from the installed app */
      scope: './',
      display: 'standalone',
      background_color: '#0f1115',
      theme_color: '#0f1115',
      icons: [icon(ICON_192, 192),
              icon(new URL('assets/icon-512.png', location.href).href, 512),
              { src: new URL('assets/icon-maskable-512.png', location.href).href, sizes: '512x512', type: 'image/png', purpose: 'maskable' }],
    };
    const link = document.createElement('link');
    link.rel = 'manifest';
    link.href = URL.createObjectURL(new Blob([JSON.stringify(manifest)], { type: 'application/manifest+json' }));
    document.head.appendChild(link);
    log('Inline PWA manifest registered (icons embedded, no extra file)');
  } catch (e) { warn('Manifest build failed (PWA features limited)', e); }
})();
