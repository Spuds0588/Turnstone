...
**☰ → ⧉ Copy share link** copies a shareable `?file=` deep link that reopens the exact same dataset — statuses ride along via browser storage on the same machine. For a list with **no address of its own** — a local pick, a drop, a paste — the same item builds a **workspace link** instead, carrying the rows and the ticks and notes entered so far; **⇱ Build a share link…** is the same thing with a preview and the columns offered as chips.
...
`turnstone-standalone.html` is the whole app in one ~1.25 MB document — the shell, PapaParse, SheetJS and the logo art, all inlined. Download it, double-click it, done: no server, no checkout, no install, no network. It is the build for **USB sticks, internal shares, and air-gapped or locked-down machines**, and for anyone who would rather keep a single file than trust a URL.
