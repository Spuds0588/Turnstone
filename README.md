...
One limit belongs here too, because it decides which edition an agent should target: a **browser side panel cannot be driven by an agent**. Working the extension's panel would take its own Chrome with the extension loaded and a panel open — which a coding agent in a sandbox, in a headless browser, or on another machine does not have — and a side panel has no address bar for a `#zdata=` link to arrive through. So an agent should hand the **extension** a data file (CSV, TSV, XLSX) and hand the **web app, standalone build or bookmarklet** a link; the versions page spells this out as a table. Nothing here is a Turnstone limitation — it is what a side panel is.
...
- **The link column is chosen from the values, not the headers.** Turnstone scores every column by what its cells actually *look like* — a URL, an `@address` or a phone number all count, weighted so a real link beats an address and an address beats a bare number — and picks the winner. A column called `Page`, `Resource` or nothing at all still lands correctly, and a notes column stuffed with pasted links can never outbid the real one (status/notes columns are skipped outright). The header text is used only to break an exact tie. If it ever guesses wrong, **⚙ Columns → the dropdown on that column → URL** pins it, and the flag is remembered **per file** (keyed to the header row) and carried inside column presets. Address and phone columns are not lost either: any cell that is an email or a number is clickable on its own card row.
...
`turnstone-standalone.html` is the whole app in one ~1.21 MB document — the shell, PapaParse, SheetJS and the logo art, all inlined. Download it, double-click it, done: no server, no checkout, no install, no network. It is the build for **USB sticks, internal shares, and air-gapped or locked-down machines**, and for anyone who would rather keep a single file than trust a URL.
